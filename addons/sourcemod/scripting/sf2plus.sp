// TODO: TEST SIXTH SENSE SOUND END AND ADD COOLDOWN END SOUND
#include <sourcemod>
#include <tf2_stocks>
...
public void OnConfigsExecuted()
{
	...
	else
	{
		...
		kv.GetString("sound", EXPInfo.PrestigeUp_Sound, PLATFORM_MAX_PATH, "items/powerup_pickup_king.wav");
...
public void CheckClientSQL(int client)
{
	if (IsValidHandle(DB))
	{
		...
		if (SQL_FastQuery(DB, query)) // If player doesn't have prefs (the row above was successfully created)
		{
			...
			FormatEx(query, sizeof(query), "UPDATE player SET hudflags='%i', chatflags='%i' WHERE id='%s'", HUDFLAGS_ALL, CHATFLAGS_ALL, ClientAuthID);
...
public void SF2PClientHudRefresh(int client)
{
	if (SF2PClientCheck(client, false, false) && ClientInfo[client].HudFlags & HUDFLAGS_ENABLEHUD || ClientInfo[client].SettingHud)
	{
		...
		for (int i; i<iActiveBosses; i++)
		{
			if (IsValidEntity(i))
			{
				if (BossInfo[i].Target == client && SF2_GetBossState(i) > 2)ClientInfo[client].Chased = true;
				if (BossInfo[i].Target == client && SF2_GetBossState(i) == 2)ClientInfo[client].Heard = true;
			}
		}
...
{
	int client = GetNativeCell(1);
	ClientInfo[client].HudFlags &~ GetNativeCell(2);
