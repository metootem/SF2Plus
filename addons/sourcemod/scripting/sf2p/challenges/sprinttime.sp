...
{
	LogMessage("[SF2+ SprintTime] Sprint Time Has Been Chosen For %N", client);
	ClientInfo[client].Selected = true;
	ClientInfo[client].Goal = GetRandomInt(min, max);
	char desc[64];
	FormatEx(desc, sizeof(desc), "Sprint For %i Seconds.", ClientInfo[client].Goal);
...
Action CheckClientProgress(Handle timer)
{
	...
	for (int i=1; i<MAXTF2PLAYERS; i++)
	{
		...
		{
			active = true;
			if (SF2_IsClientReallySprinting(i))
			{
				ClientInfo[i].Progress += tick;
				char desc[64];
				FormatEx(desc, sizeof(desc), "Sprint For %0.1f/%i Seconds.", ClientInfo[i].Progress, ClientInfo[i].Goal);
