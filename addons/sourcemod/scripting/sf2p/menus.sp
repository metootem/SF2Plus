...
void Menu_BossInfo(int client, int target)
{
	...
	if (IsValidEntity(ent))
	{
		...
		if (SF2_GetBossType(target) ==  SF2BossType_AdvancedChaser)FormatEx(title, sizeof(title), "%sType: Advanced Chaser\n", title, buffer); 
		FormatEx(title, sizeof(title), "%s \n", title);
		...
		if (SF2_IsBossStunnable(target))
		{
			...
			FormatEx(title, sizeof(title), "%sStun Cooldown: %0.2f\n", title, (time <= 0.0 ? 0.0 : time));
		}
